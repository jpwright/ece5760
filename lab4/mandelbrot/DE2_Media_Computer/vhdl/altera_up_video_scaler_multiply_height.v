...
begin
	if (reset)
		present_state <= STATE_0_GET_CURRENT_LINE;
	else
...
begin
	...
	STATE_0_GET_CURRENT_LINE:
		begin
			if (width_in == WIDTH)
				next_state = STATE_1_LOOP_FIFO;
			else
				next_state = STATE_0_GET_CURRENT_LINE;
		end
	STATE_1_LOOP_FIFO:
		begin
			if (fifo_read & (width_out == (WIDTH - 1)) & 
					(&(enlarge_height_counter | 1'b1)))
				next_state = STATE_2_OUTPUT_LAST_LINE;
			else
				next_state = STATE_1_LOOP_FIFO;
		end
...
begin
	...
	begin
		...
		if (present_state == STATE_0_GET_CURRENT_LINE)
			stream_out_valid			<= 1'b0;
		else
...
begin
	if (reset)
		width_in <= 'h0;
	else if (present_state == STATE_1_LOOP_FIFO)
		width_in <= 'h0;
	else if (stream_in_ready & stream_in_valid)
		width_in <= width_in + 'h1;
end
...
begin
	...
	else if (present_state == STATE_0_GET_CURRENT_LINE)
		enlarge_height_counter <= 'h0;
	else if (fifo_read & (width_out == (WIDTH - 1)))
		enlarge_height_counter <= enlarge_height_counter + 'h1;
end
...
assign stream_in_ready			= 
		(present_state == STATE_1_LOOP_FIFO) ? 
			1'b0 : 
