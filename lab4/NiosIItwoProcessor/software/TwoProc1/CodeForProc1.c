...
/*the hardware mutex device handle */
alt_mutex_dev* mutex ;
...
begin
  ...
  /*get hardware mutex device handle */
  ...
  begin
    //read the switches
    sw = IORD_ALTERA_AVALON_PIO_DATA(IN_0_BASE);
    if (sw & 0x80) //if sw15 is up, then get mutex   
      /* acquire the mutex, setting the value to one */
      altera_avalon_mutex_lock( mutex, 1 );
    else /* release the lock */
      altera_avalon_mutex_unlock( mutex );   
    //display current count
    IOWR_ALTERA_AVALON_PIO_DATA(OUT_0_BASE, flag);   
  end
end
