...
void interval_timer_isr(int *, int *, int []);

/* global variables */
extern int	display_buffer[];
extern int	eight_sec;
extern int	green_LED_pattern;
extern int	shift_direction;
...
void interrupt_handler(void)
{
	...
	{
		++eight_sec;
		interval_timer_isr( & green_LED_pattern, & shift_direction, display_buffer );
	}
	if ( ipending & 0x2 )				// pushbuttons are interrupt level 1
	{
		pushbutton_isr( );
	}
	if ( ipending & 0x1000 )			// JP2 is interrupt level 12
	{
		// do nothing; this is just a placeholder for JP2 expansion port interrupts
	}
